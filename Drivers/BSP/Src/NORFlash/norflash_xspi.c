#include "bsp_norflash_xspi.h"

void NORFlash_XSPI_Init(NORFlash_XSPI_ObjectTypeDef *XSPIObject,
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_SetClock(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint32_t ClockInput,
		uint32_t ClockRequested, uint32_t *ClockReal) {
	uint32_t Divider;
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_EnableMapMode(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t CommandRead,
		uint8_t DummyRead, uint8_t CommandWrite, uint8_t DummyWrite) {
	...
	Cmd.Instruction =
			(Cmd.InstructionWidth == HAL_XSPI_INSTRUCTION_16_BITS) ?
					(((uint16_t) CommandRead << 8) | (uint8_t) (~CommandRead & 0xFF)) :
					CommandRead;
	Cmd.DummyCycles = DummyRead;
	if (HAL_XSPI_Command(XSPIObject->XSPIHandle, &Cmd,
	...
	Cmd.Instruction =
			(Cmd.InstructionWidth == HAL_XSPI_INSTRUCTION_16_BITS) ?
					(((uint16_t) CommandWrite << 8) | (uint8_t) (~CommandWrite & 0xFF)) :
					CommandWrite;
	...
	Error: HAL_XSPI_Abort(XSPIObject->XSPIHandle);
	return NORFlash_XSPI_ERROR;
}

NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_DisableMapMode(
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_ConfigPHYLink(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject,
	...
	case NORFlash_PHY_LINK_1S1S1S: {
		XSPIObject->BaseCommand.InstructionMode = HAL_XSPI_INSTRUCTION_1_LINE;
		XSPIObject->BaseCommand.InstructionWidth = HAL_XSPI_INSTRUCTION_8_BITS;
		XSPIObject->BaseCommand.InstructionDTRMode =
	...
	Error: return NORFlash_XSPI_ERROR;
}

NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_CommandSendData(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint8_t *Data,
		uint16_t DataSize) {
	XSPI_RegularCmdTypeDef Cmd = XSPIObject->BaseCommand;

	Cmd.Instruction =
			(Cmd.InstructionWidth == HAL_XSPI_INSTRUCTION_16_BITS) ?
					(((uint16_t) Command << 8) | (uint8_t) (~Command & 0xFF)) : Command;
	...
	if (DataSize != 0) {
		if (HAL_XSPI_Transmit(XSPIObject->XSPIHandle, Data,
				HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
			goto Error;
		}
	}
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_CommandSendAddress(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint32_t Address) {
	XSPI_RegularCmdTypeDef Cmd = XSPIObject->BaseCommand;
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_CommandRead(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint8_t *Data,
		uint16_t DataSize) {
	XSPI_RegularCmdTypeDef Cmd = XSPIObject->BaseCommand;
	...
	if (DataSize != 0) {
		if (HAL_XSPI_Receive(XSPIObject->XSPIHandle, Data,
				HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
			goto Error;
		}
	}
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_CommandSendAddressReadData(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint32_t Address,
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_CommandSendAddressSendData(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint32_t Address,
		uint8_t *Data, uint16_t DataSize) {
	XSPI_RegularCmdTypeDef Cmd = XSPIObject->BaseCommand;
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_WaitStatusRegister(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command,
		uint16_t MatchValue, uint16_t MatchMask, uint8_t DataSize, uint32_t Timeout) {
	XSPI_RegularCmdTypeDef Cmd = XSPIObject->BaseCommand;
	...
	Cmd.AddressMode =
			(Cmd.InstructionMode == HAL_XSPI_INSTRUCTION_8_LINES) ?
					HAL_XSPI_ADDRESS_8_LINES : HAL_XSPI_ADDRESS_NONE;
...
NORFlash_XSPI_StatusTypeDef NORFlash_XSPI_Write(
		NORFlash_XSPI_ObjectTypeDef *XSPIObject, uint8_t Command, uint8_t Dummy,
