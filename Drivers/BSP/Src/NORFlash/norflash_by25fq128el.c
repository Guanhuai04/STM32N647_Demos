...
static NORFlash_StatusTypeDef BY25FQ128EL_Init(
		NORFlash_ObjectTypeDef *NORFlashObject, uint32_t ClockInput,
		uint32_t *ClockRequested, uint8_t IsDualChip) {
	uint8_t Data[2];
	...
	NORFlash_XSPI_CommandRead(&(NORFlashObject->XSPIObject), 0x35, Data,
			DataSize);
	if (((Data[0] & (1 << 1)) != (1 << 1))
			|| (IsDualChip && ((Data[1] & (1 << 1)) != (1 << 1)))) {
		Data[0] |= (1 << 1);
		Data[1] |= (1 << 1);

		NORFlash_EnableWrite(NORFlashObject, 1000);

		NORFlash_XSPI_CommandSendData(&(NORFlashObject->XSPIObject), 0x31, Data,
				DataSize);
	...
	if (NORFlash_XSPI_CommandSendData(&(NORFlashObject->XSPIObject), 0x38, NULL,
			0) != NORFlash_XSPI_OK) {
		return NORFlash_ERROR;
	}
	...
	if (NORFlash_XSPI_CommandSendData(&(NORFlashObject->XSPIObject), 0xC0, Data,
			DataSize) != NORFlash_XSPI_OK) {
		return NORFlash_ERROR;
	}
#endif
...
NORFlash_StatusTypeDef NORFlash_BY25FQ128EL_Init(
		NORFlash_ObjectTypeDef *NORFlashObject, uint32_t ClockInput,
		uint32_t *ClockRequested) {
	return BY25FQ128EL_Init(NORFlashObject, ClockInput, ClockRequested, 0);
}

NORFlash_StatusTypeDef NORFlash_DUAL_BY25FQ128EL_Init(
		NORFlash_ObjectTypeDef *NORFlashObject, uint32_t ClockInput,
		uint32_t *ClockRequested) {
